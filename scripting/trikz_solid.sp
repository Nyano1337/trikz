...
Handle g_hPassServerEntityFilter;
...
public void OnPluginStart()
{
    ...
    Handle hFunctions = LoadGameConfigFile("trikz_solid.games");
    if (!hFunctions)
        SetFailState("Failed to load ftrikz_solid gamedata");
        
    g_hPassServerEntityFilter = DHookCreateFromConf(hFunctions, "PassServerEntityFilter");
    if(!g_hPassServerEntityFilter)
    {
        delete hGameData;
        delete hFunctions;
        SetFailState("Failed to setup detour for PassServerEntityFilter");
    }
    
    if(!DHookEnableDetour(g_hPassServerEntityFilter, false, PassServerEntityFilter))
    {
        delete hGameData;
        delete hFunctions;
        SetFailState("Failed to detour PassServerEntityFilter.");
    }
...
public OnTrikzNewPartner(int client, int partner)
{
    for(int i = 0; i < g_CurrentSolid; i++)
    {
        ...
        g_iOldToggles[i][client] = g_iAllowedToggles[i][partner];
...
public MRESReturn AcceptInputCounter(int pThis, Handle hReturn, Handle hParams)
{
    ...
    int partner = Timer_GetPartner(activator);
    if(partner == 0)
        activator = 0;
...
bool FindAllSolidsWithOutput(int entity, const char[] output)
{
    ...
    int count = GetOutputCount(entity, sOutput);
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        GetOutputTargetInput(entity, sOutput, i, input, sizeof(input));
        ...
        {
            GetOutputTarget(entity, sOutput, i, target, sizeof(target));
...
public void OnToggleEntity(const char[] output, int caller, int activator, float delay)
{
    ...
    for(int i = 0; i < 12; i++)
    {
        if(g_iLinkedSolids[rEnt][i] != -1)
        {
            ...
            int partner = Timer_GetPartner(person);
...
void ProccessMathCounterWithName(const char[] sTargetname)
{
    ...
    if((GetOutputCount(entity, "m_OutValue") > 0) || (GetOutputCount(entity, "m_OnGetValue") > 0) || (GetOutputCount(entity, "m_OnUser3") > 0) || (GetOutputCount(entity, "m_OnUser4") > 0)) //this shit is bananas b a n a n a s
        return;
    ...
    count = GetOutputCount(entity, "m_OnHitMin");
...
void ProccessBreakableEntitiesWithName(const char[] sTargetname, const char[] sClassname)
{
    ...
    while((iTarget = FindEntityByTargetname(iTarget, sTargetname, sClassname)) != INVALID_ENT_REFERENCE)
    {
        if(!g_bUsedEdict[iTarget])
        {
            ...
            int count = GetOutputCount(iTarget, "m_OnBreak");
...
public Action Output_OnStartTouch(const char[] output, int caller, int activator, float delay)
{
    ...
    if(Timer_GetPartner(ractivator) == 0)
        ractivator = 0;
...
/*public Action Hook_StartTouch(int entity, int other)
{
    ...
    if(Timer_GetPartner(activator) == 0)
        activator = 0;
...
public MRESReturn PassServerEntityFilter(Handle hReturn, Handle hParams)
{
    ...
    if(ent1 == ent2 || !ent1 || !ent2 || !IsValidEntity(ent1) || !IsValidEntity(ent2))
        return MRES_Ignored;
        
    //check logic from other plugins first!!!!
